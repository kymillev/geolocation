...
def build_model(alphabet,
    ...
    prediction_model = keras.models.Model(inputs=inputs, outputs=[CTCDecoder()(model.output),model.output])
...
class Recognizer:
    ...
    def recognize(self, image):
        ...
        preds = self.prediction_model.predict(image[np.newaxis])
        preds_idx = preds[0][0]
        preds_softmax = preds[1][0]
        
        # Estimation of confidence, should be correct?
        conf = np.max(preds_softmax,axis=1).cumprod()[-1]
        return ''.join([
            self.alphabet[idx] for idx in preds_idx
    ...
    def recognize_from_boxes(self, images, box_groups, **kwargs) -> typing.List[str]:
        ...
        if len(X.shape) == 3:
            X = X[..., np.newaxis]
        print('Recognition preprocess:',round(time.time()-t1,3),'s')
        t1 = time.time()
        preds_raw = self.prediction_model.predict(X, **kwargs)
        preds_idx = preds_raw[0]
        preds_softmax = preds_raw[1]
        
       
        predictions_str = [''.join([self.alphabet[idx] for idx in row if idx not in [self.blank_label_idx, -1]])
            for row in preds_idx]
        predictions_conf = [np.max(prob_arr,axis=1).cumprod()[-1] for prob_arr in preds_softmax]
         
        return [list(zip(predictions_str[start:end],predictions_conf[start:end])) for start, end in start_end]
    
    
    def recognize_from_boxes_batch(self, images, box_groups, **kwargs) -> typing.List[str]:
        """Recognize text from images using lists of bounding boxes.
        ...
        preds = [r for r in self.prediction_model.predict(X, **kwargs)]
        ...
        print('Recognition predict:',round(time.time()-t1,3),'s')
